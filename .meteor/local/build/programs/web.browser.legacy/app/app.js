...
Template.App_page2.events({
  ...
  "change .custom-checkbox": function (evt) {// TODO: event target trp children for loop foreach td innerHTML
...
var Video = new Mongo.Collection('video');
...
Meteor.methods({
  "video.insert": function (sid, rid, message, type) {
    var uid = Video.insert({
      sid: sid,
      rid: rid,
      message: message,
      viewed: false,
  ...
  "video.update": function (videoId, update) {
    return Video.update(videoId, {
      $set: update
    });
  }
