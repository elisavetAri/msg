...
module.export({
  ...
  cleanFieldNameForSearch: () => cleanFieldNameForSearch,
  sortsAreEqual: () => sortsAreEqual,
  objectsAreEqual: () => objectsAreEqual,
  getMongoSort: () => getMongoSort
});
...
function cleanFieldNameForSearch(field) {
  ...
  if (field.indexOf('\"') !== -1) {
    console.warn("The column data value '".concat(field, "' contains a \" character and will not be properly parsed for enabling search"));
...
function getMongoSort(order, columns) {
  ...
  _.each(order, (_ref) => {
    ...
    const column = columns[colIndex]; // Sometimes when swapping out new table columns/collection, this will be called once
